...
class Model(nn.Module):
	...
	def forward(self, chunks_idx , tok2sent_ind ,  labels_idx , tok2label, tokenizer , print_att = False):
		...
		input_tokens =  [  [101] + t +  [102]    for t in chunks_idx] #0 vs 2 for RoBerTA	
		...
		num_cluster = 4
		if toks_embed.shape[0] >= num_cluster	:

			gmm_model = GaussianMixture(n_components = num_cluster ,  n_features=768 , covariance_type="diag", eps=1.e-3, ).to(self.device)
			gmm_model.fit(toks_embed)
			cluster_centers = gmm_model.mu.squeeze(0)
			# print('cluster center shape ' , cluster_centers.shape)
		else:
			cluster_centers = torch.rand( num_cluster , toks_embed.shape[1])
