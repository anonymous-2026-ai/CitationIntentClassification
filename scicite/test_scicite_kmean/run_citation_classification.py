	...
	for _ in range(int(args.num_train_epochs)):
		...
		for step, batch_index in enumerate(list_index_batch):
			...
			if args.classification_type == "multiclass":
				...
				sample = features[batch_index[0]]
				
				tok2sent ,  chunks_idx , tok2label , labels_idx , label_binary = sample[0] , sample[1] , sample[3], sample[4], sample[-1]
				# if step % 50 == 0 :
				# 	logits = model(chunks_idx , tok2sent , labels_idx , tok2label, tokenizer , print_att=True)
				# else:
			...
			if args.logging_steps > 0 and global_step % args.logging_steps == 0:
				...
				# file = open(save_output + '/checkpoint_'+str(global_step) , 'wb')
				# import pickle
				# pickle.dump(model , file)
				if results['strict'] >= bestS and results['weak'] >= bestW:
...
def main():
	...
	print('save output ', save_output)
	os.makedirs(save_output)
	results = evaluate(args, model, tokenizer, processor = processor , save_output = save_output, global_step= 0)
	print(results)
