    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.classification_type == "multiclass":
                ...
                # print('labels ' , batch[3].type_as(logits))
            ...
            if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                ...
                results = evaluate(args, model, tokenizer, processor = processor , global_step= global_step)
                print('results and eval loss ', results)
...
def evaluate(args, model, tokenizer, prefix="", processor=None, global_step = 0 ):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.do_eval and not args.do_test:
            ...
            output_eval_file = os.path.join(eval_output_dir, prefix, "test_results"+str(global_step)+".txt")
            output_predictions_file = os.path.join(eval_output_dir, prefix, "predictions"+str(global_step)+".txt")
            output_predictions_probs_file = os.path.join(eval_output_dir, prefix, "predictions_probs"+str(global_step)+".txt")
...
def main():
    ...
    model = Model(pretrained_model , 768 , 7 , args.device)
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, processor = processor)
