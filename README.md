...
Step 1: Select the checkpoints that give the best performances on the development sets and transfer them from the train_folder to the test_folder for evaluation. You also have to change the path to your dataset with '--data_dir' parameter in 'run.sh' file. Note that, for the MultiCite dataset, we select the checkpoint for testing if it gives the highest score in both strict and weak metrics on the MultiCite development set. In the ACL-ARC and SciCite datasets, we select the checkpoint for testing if it gives the best score in micro-F1 on the development sets of ACL-ARC and SciCite datasets, respectively. 
